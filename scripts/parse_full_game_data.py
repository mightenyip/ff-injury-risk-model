#!/usr/bin/env python3
"""
Parse the full game data from PFR CSV files.
The CSV files have many more columns than initially detected.
"""
...
def parse_full_csv(file_path):
    """Parse a PFR CSV file with all columns."""
    print(f"Parsing {os.path.basename(file_path)}...")
    
    try:
        # Read the CSV with header=None to get all columns
        ...
        print(f"  ✓ Parsed {len(data_df)} games with {len(column_names)} columns")
...
def main():
    """Main function to parse all PFR data with full columns."""
    print("Parsing Full PFR Game Data")
    print("=" * 35)
    
    # Input and output directories
    input_dir = "data/weekly_raw"
    output_dir = "data/full_game_data"
    ...
    for csv_file in csv_files:
        parsed_df = parse_full_csv(csv_file)
    ...
    output_file = os.path.join(output_dir, "full_game_data.csv")
    combined_df.to_csv(output_file, index=False)
    
    print(f"\n✅ Successfully parsed full game data!")
